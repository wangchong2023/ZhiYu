...
| 特性 | 状态 | 说明 |
| :--- | :--- | :--- |
| **3D 动力学图谱** | ✅ 已上线 | `GraphView` + `GraphViewModel` 支持万级节点力导向渲染 |
...
| **Spotlight 全局搜索集成** | macOS | 🚧 开发中 | `Infrastructure/Storage/Search/SpotlightService.swift` 存在，集成完整度待验证 |
...
| **隐私模式（高斯模糊）** | ✅ 已上线 | `#private` 标签触发模糊处理，`SettingsStore.isPrivacyModeEnabled` 实现 |
...
| **撤销/重做** | ✅ 已上线 | 系统 `UndoManager` 全库操作历史支持多级回退 |

## 6. 知识维护与质量 (Maintenance & Quality)

| 特性 | 状态 | 说明 |
| :--- | :--- | :--- |
| **健康检查 (Lint)** | ✅ 已上线 | 断链/孤岛/循环引用检测，`LintService` 完整实现 |
| **勋章/游戏化系统** | 🚧 开发中 | `MedalService.swift` 已实现核心逻辑（`Sources/Features/Insight/MedalWall/Service/`），需补充触发逻辑和奖励 UI 面板 |
| **操作日志 (Activity Log)** | ✅ 已上线 | `LogEntry` 记录全类型操作，`LogView` 可视化追溯（`Sources/Features/Insight/Log/View/`） |
...
| **PDF 解析** | ✅ 已上线 | 原生 PDF 读取与结构化提取，`PDFReaderView` + `PDFServiceProtocol` 完整实现 |
...
| 特性 | 状态 | 说明 |
| :--- | :--- | :--- |
| **Vision Pro 空间视图** | 🚧 开发中 | `VisionProSpatialView.swift` 仅提供降级 2D 占位，需引入 RealityKit 落地 3D 图谱空间交互 |
...
---
*本文档状态标注基于 2026-09-25 全量代码扫描，随版本迭代持续更新。*
