...
| 17 | 🔴 P0 | `Sources/Infrastructure/Storage/Repositories/DatabaseWriterProvider.swift:24-36` | 静默降级导致数据丢失 | `DatabaseWriterProvider.dbWriter` 默认实现：当 `DatabaseManager.shared.dbWriter` 为 nil 时（如启动早期、Vault 热切换瞬态、reset 后），静默创建空内存 `DatabaseQueue()` 并返回。调用方（各 Repository）无感知，所有读写操作写入临时内存库，数据在进程结束或下次切换后丢失 | 启动早期或热切换瞬态窗口期的所有数据写入静默丢失：用户新建/编辑的知识页、标签、链接等全部消失，无任何错误提示。极端情况下用户可能丢失大量工作内容 | 移除静默降级，改为抛出明确错误（如 `DatabaseError.notReady`），由调用方决定降级策略（如排队等待、提示用户）；或在降级时记录严重日志并广播通知 | 待确认 | 批次6-B |
