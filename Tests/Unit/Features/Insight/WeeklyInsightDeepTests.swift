//
//  WeeklyInsightDeepTests.swift
//  ZhiYuTests
//
//  合并自 2 个碎片化测试文件：TagCloudAndWeeklyInsightDeepTests.swift, WeeklyInsightCardDeepTests.swift
//
...
@MainActor
final class WeeklyInsightDeepTests: XCTestCase {
    ...
    func testTagCloudEmptyStateAndBubbleRatio() throws {
        ...
        XCTAssertEqual(coordinator.tags.count, 0, "空状态下 coordinator 标签列表应为空")
        XCTAssertNil(coordinator.selectedTag, "空状态下不应有选中标签")
    }
    ...
    func testWeeklyInsightCardWithInsightData() throws {
        ...
        host.view.layoutIfNeeded()
        XCTAssertNotNil(host.view)
        XCTAssertEqual(aiStore.weeklyInsight?.totalNewPages, 15)
    ...
    func testWeeklyInsightCardEmptyState() throws {
        ...
        host.view.layoutIfNeeded()
        XCTAssertNotNil(host.view)
        XCTAssertNil(aiStore.weeklyInsight, "空状态下 weeklyInsight 应为 nil")
    }
