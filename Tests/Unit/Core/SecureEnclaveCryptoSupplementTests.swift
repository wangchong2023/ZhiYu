...
final class SecureEnclaveCryptoSupplementTests: XCTestCase {
    ...
    func testEncryptFallbackPathCiphertextShouldNotBeEmptyString() throws {
    ...
    func testDecryptMigrationPathReencryptResultDiscarded() {
    ...
    /// 但通过源码审查确认逻辑缺陷）。
    func testGetOrCreateHKDFSaltKeychainReturnsNilGeneratesNewSalt() {
    ...
    func testGetOrCreateHardwarePrivateKeyTokenCorruptedSilentRebuild() {
        // 此方法在模拟器降级路径下不会被调用
    ...
    /// 发现目的：验证降级路径的级联错误传播。
    func testEncryptFallbackPathSecurityManagerFailureErrorPropagation() {
    ...
    func testDecryptFallbackPathSecurityManagerFailureErrorPropagation() {
        let failingMock = FailingSecurityManager()
        SecurityManager.testOverride = failingMock
    ...
    /// 发现目的：验证密钥不会因实例重建而变化
    func testEncryptDecryptCrossInstanceKeyConsistent() throws {
    ...
    /// 发现目的：验证加密输出的格式稳定性
    func testEncryptMultipleTimesCiphertextFormatConsistent() throws {
    ...
    func testEncryptFallbackPathNoKeychainSecureEnclaveDataWritten() throws {
    ...
    /// 空字符串加密后解密应还原空字符串
    /// 发现目的：验证空明文的环回正确性
    func testEncryptDecryptEmptyPlaintextRoundtripCorrect() throws {
    ...
    func testEncryptDecryptWithNullBytesRoundtripCorrect() throws {
    ...
    func testEncryptCiphertextBytesExcludePlaintextByteSequence() throws {
