...
final class AuthIntegrationTests: XCTestCase {
    ...
    override func setUpWithError() throws {
        // 强制关闭 Mock，走真实后端网络
        MainActor.assumeIsolated { AuthService.forceMockBackend = false }
    }
    ...
    func testCarrierLoginIntegration() async throws {
        ...
        print(">>> [Integration] Carrier Login Success: \(success ?? false)")
    }
    ...
    func testGoogleLoginIntegration() async throws {
        ...
        print(">>> [Integration] Google Login Success: \(success ?? false)")
    }
    ...
    func testAppleLoginIntegration() async throws {
        ...
        print(">>> [Integration] Apple Login Success: \(success ?? false)")
    }
    ...
    func testGitHubLoginIntegration() async throws {
        ...
        print(">>> [Integration] GitHub Login Success: \(success ?? false)")
    }
    
    /// 超时工具：限制异步操作最长执行时间，超时返回 false
    private func withTimeout<T>(seconds: TimeInterval, operation: @escaping @Sendable () async -> T) async -> T? where T: ExpressibleByBooleanLiteral, T: Sendable {
